...
export function renderOrderSummary() {
    ...
    function deliveryOptionHTML(matchingProduct, cartItem) {
      ...
      deliveryOptions.forEach((deliveryOption) => {
        ...
        const priceString = deliveryOption.priceCents === 0 ? 'FREE' :
          `${formatPrice(deliveryOption.priceCents)} -`; // <-- UPDATED
    ...
    // the event listeners for the delete link

    document.querySelectorAll('.js-delete-link')
      .forEach((link) => {
        link.addEventListener('click', () => {
          const productId = link.dataset.productId;
          // when we click delete we should update the data (Model) and then regenerate the HTML (View)
          removeFromCart(productId);
          // regenerate the HTML according to MVC instead of using the DOM
    ...
    // the event listeners for the update and save links
      ...
      document.querySelectorAll('.delivery-option-input')
        .forEach((element) => {
          element.addEventListener('click', () => {
            ...
            renderOrderSummary();
            // regenerate the HTML according to MVC
            renderPaymentSummary();
          });
        });
  }